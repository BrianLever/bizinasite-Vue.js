...
class UserMeeting extends BaseModel
{
    ...
    public function getAvailablePeriod($date, $category)
    {
        ...
        if($b_hours1!=null&&$b_hours1->whole_date==1) return [];
        if($b_hours2!=null&&$b_hours2->whole_date==1) return [];
        ...
        do {
            $v1 = 0;$v2 = 0;$v3 = 0;$b1 = 1;$b2 = 1;;$b3 = 1;
            foreach($a_hours1 as $a_h)
            {
                if(timePeriodCompare($start, $end, $a_h->start, $a_h->end)==1) $v1=1;
            }
            foreach($a_hours2 as $a_h2)
            {
                if(timePeriodCompare($start, $end, $a_h2->start, $a_h2->end)==1) $v2=1;
            }
            foreach($a_hours3 as $a_h3)
            {
                if(timePeriodCompare($start, $end, $a_h3->start, $a_h3->end)==1) $v3=1;
            }
            if($b_hours1!=null)
            {
                foreach(json_decode($b_hours1->start) as $key1=>$s1)
                {
                    if(timePeriodCompare($start, $end, $s1, json_decode($b_hours1->end)[$key1])==1)  $b1 = 0;
                }
            }
            if($b_hours2!=null)
            {
                ...
                {
                    if(timePeriodCompare($start, $end, $s2, json_decode($b_hours2->end)[$key2])==1)  $b2 = 0;
                }
            }
            if($b_hours3!=null)
            {
                ...
                {
                    if(timePeriodCompare($start, $end, $s3->start_time, $s3->end_time)==1)  $b3 = 0;
                }
            }
