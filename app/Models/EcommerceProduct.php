    ...
    public function getAllVariants()
    {
        ...
        if($this->size&&count($sizes)) {
                ...
                if ($this->color && count($colors)) {
                    foreach ($colors as $color) {
                        if ($this->variants && count($variants))
                            foreach ($variants as $variant) { array_push($result, $this->makeItem($size, $color, $variant));}
                        else array_push($result, $this->makeItem($size, $color, null));
                    }
                }
                else {
                    if($this->variants&&count($variants))
                        foreach($variants as $variant){ array_push($result, $this->makeItem($size, null, $variant)); }
                    else array_push($result, $this->makeItem($size, null, null));
                }
            }
        }
        else {
            if($this->color&&count($colors))
            {
                ...
                {
                    if($this->variants&&count($variants))
                        foreach($variants as $variant) { array_push($result, $this->makeItem(null, $color, $variant));}

                    else array_push($result, $this->makeItem(null, $color, null));
                }
            }
            else {
                if($this->variants&&count($variants))
                    foreach($variants as $variant){ array_push($result, $this->makeItem(null, null, $variant));}
            }
        }
        return $result;
    }
