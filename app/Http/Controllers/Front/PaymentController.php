...
class PaymentController extends Controller
{
    ...
    public function stripeExecute(Request $request)
    {
        try{
            ...
            if($validation->fails()) return response()->json(['status'=>2, 'data'=>$validation->errors()]);
    ...
    public function paypalExecute(Request $request)
    {
        ...
        if(empty($token)) return redirect()->route('cart.checkout')->with("error", 'Session is expired');
