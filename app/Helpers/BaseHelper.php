...
if (! function_exists('tenant')) {
    ...
    function tenant($key=null)
    {
        try {
            $tenant = app('tenant');
            if($key=='c_modules') return $tenant->getModules->where("status", 1)->pluck("slug")->toArray();
            if($key=='p_modules') return $tenant->getModules->where("status", 1)->where("publish", 1)->pluck("slug")->toArray();
...
if (! function_exists('checkMark')) {
    ...
    {
        if ($value) return asset('assets/img/checked.png');
        else return asset('assets/img/remove1.png');
    }
}
...
function deploy_key(): string{
    return 'sha256-'.hash('sha256', config('app.version'));
}
